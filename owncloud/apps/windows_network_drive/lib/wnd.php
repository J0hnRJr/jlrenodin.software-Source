...
class WND extends StorageAdapter {
	...
	protected $password;
	protected $root;
	protected $isKerberos = false;
	protected $userCCache;
	...
	public function __construct($params) {
		...
		if (!isset($params['user'], $params['password'], $params['host'], $params['share']) &&
			!isset($params['user'], $params['kerberosServerId'], $params['host'], $params['share'])
		) {
		...
		if (isset($params['kerberosServerId'])) {
			$this->isKerberos = true;
			$kerberosServerId = $params['kerberosServerId'] ?? '';
			$ccacheHandler = CCacheHandler::getCCacheHandlerFromConfig($config);

			$kerberosUser = $ccacheHandler->getMappedUser($this->user, $kerberosServerId);
			$this->userCCache = $ccacheHandler->retrieveUserCCache($kerberosUser, $kerberosServerId);
			if ($this->userCCache === false) {
				$serverCCache = $ccacheHandler->retrieveServerCCache($kerberosServerId);
				if ($serverCCache === false) {
					$serverCCache = $ccacheHandler->createServerCCache($kerberosServerId);
				}
				$this->userCCache = $ccacheHandler->createUserCCache($serverCCache, $this->host, $kerberosUser, $kerberosServerId);
			}
			// The required KRB5CCNAME env var will be set within the smbclientWrapper
			// using the "ccache" parameter set in the opts

			$smbclientWrapperOpts = [
				SMBCLIENT_OPT_TIMEOUT => $config->getSystemValue('wnd.connector.opts.timeout', SmbclientWrapper::OPTS_TIMEOUT),
				SMBCLIENT_OPT_USE_KERBEROS => true,
				'ccache' => $this->userCCache,
			];
		} else {
			$smbclientWrapperOpts = [
				SMBCLIENT_OPT_TIMEOUT => $config->getSystemValue('wnd.connector.opts.timeout', SmbclientWrapper::OPTS_TIMEOUT),
	...
	private function connectionTestWithReset() {
		...
		try {
			...
			if (($e->getCode() === 13 || $e->getCode() === 1) && $this->password !== '') {
				// code 13 -> permission denied -> need to change the password
				// code 1 -> operation not permitted -> "SMBCLIENT_OPT_AUTO_ANONYMOUS_LOGIN" set to
				// false and a wrong password might cause this error code (checked against windows server 2019,
				// init smb connection with wrong password + set option to false + call stat = error code 1)
				//
	...
	public function validateConfiguration() {
		...
		if (($this->getPassword() === '' && $this->getUser() !== '') && !$this->isKerberos) {
			throw new StorageNotAvailableException('Password required when username given');
		}

		if ($this->isKerberos && !\is_string($this->userCCache)) {
			throw new StorageNotAvailableException("Kerberos credential cache for {$this->user} not properly configured");
		}
	...
	protected function logEnter($funcName, $params) {
		if ($this->logActive) {
			$this->logger->debug("enter: $funcName, params = ".\json_encode($params), ['app' => 'wnd']);
		}
	}
	...
	protected function logLeave($funcName, $result) {
		if (!$this->logActive) {
			...
			$this->logger->debug("leave: $funcName, return " . \json_encode($result), ['app' => 'wnd']);
		}
		return $result;
	}
